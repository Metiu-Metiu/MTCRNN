...
MODEL="2021-04-13_ChitraGanSynth_2x2TrumpetFlute"   #"ChitraGanNsynth"

############# short and small
       #OUTDIR=$MODEL.20210330 # short run
       #LAYERSIZE=128
OUTDIR=$MODEL.20210413 # long run
LAYERSIZE=64
...
AUDIO_OUT="output/${OUTDIR}/audio/linVsin." #${c}."
AUDIO_OUT+=$STEP

echo "AUDIO_OUT = $AUDIO_OUT"

	echo "call python generate with step $STEP"
	python3 generate.py --hidden_size $LAYERSIZE --batch_size 1 --seq_len $SL --length $LEN --param_dir $DATADIR/$MODEL \
